/**
 * CRT Post-Processing GLSL ES 3.00 Shaders
 *
 ...
 *   distance squared), combined lighting mask (flicker + vignette in single
 *   multiply).
 *
 ...
 * Output goes to the fragment shader output which the browser composites as sRGB
 ...
 *
 * All shaders use GLSL ES 3.00 (#version 300 es) for WebGL 2.0.
 */
 ...
 */
export const CRT_VERTEX_SRC = `#version 300 es
in vec2 a_position;
out vec2 v_texCoord;
void main() {
 ...
 */
export const CRT_FRAGMENT_SRC = `#version 300 es
...
void main() {
  ...
  if (u_bfiStrength > 0.0001) {
    if (u_aberration > 0.0001) {
      ...
      vec3 p2 = vec3(texture(u_framePrev2, uvR).r, texture(u_framePrev2, uv).g, texture(u_framePrev2, uvB).b);
      vec3 p1 = vec3(texture(u_framePrev1, uvR).r, texture(u_framePrev1, uv).g, texture(u_framePrev1, uvB).b);
      vec3 p0 = vec3(texture(u_frameCurr,  uvR).r, texture(u_frameCurr,  uv).g, texture(u_frameCurr,  uvB).b);
      col = bfiCompute(p2, p1, p0);
    } else {
      // BFI only: 3 reads (one per history texture)
      col = bfiCompute(
        texture(u_framePrev2, uv).rgb,
        texture(u_framePrev1, uv).rgb,
        texture(u_frameCurr, uv).rgb
      );
    }
  } else if (u_aberration > 0.0001) {
    // Aberration only: 3 reads from u_texture
    col.r = texture(u_texture, uv + vec2(u_aberration, 0.0)).r;
    col.g = texture(u_texture, uv).g;
    col.b = texture(u_texture, uv - vec2(u_aberration, 0.0)).b;
  } else {
  ...
  // Input from texture() is sRGB-encoded (RGBA texture, no hw decode).
